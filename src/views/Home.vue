...
const circleAnim = () => anime({
  ...
  duration: 10000,
  translateY: () => anime.random(-270, 270),
  translateX: () => anime.random(-270, 270),
  complete: circleAnim
})
...
<template>
  <div class="hero px-0 top-0">
    <div class="grid grid-rows-[max-content,auto] overflow-hidden">
      <div class="relative -m-4 col-span-full row-span-full z-[-1]">
        <div class="bg-blue-100/40 backdrop-blur-3xl absolute inset-0 z-[1]"></div>
        <div class="relative xl:w-[Min(1200px,100%)] xl:mx-auto h-full">
          <div class="circle top-[-20%] left-[-12%] bg-blue-400 w-80 h-80"></div>
          <div class="circle top-1/2 left-[10%] bg-indigo-300 w-96 h-96"></div>
          <div class="circle top-[40%] right-[10%] bg-[#EA7FED] w-96 h-96"></div>
          <div class="circle top-[-30%] right-[25%] bg-[#EA7FED] w-80 h-80"></div>
          <div class="circle top-[-10%] right-[-20%] bg-[#60EBBD] w-96 h-96"></div>
        </div>
      </div>
      <div class="flex pt-10 space-x-4 row-start-1 col-span-full place-self-center">
        <svg class="fill-current h-12" viewBox="0 0 600 566" xmlns="http://www.w3.org/2000/svg">
          <path
        ...
        <slide-reveal-text as="h2" class="hidden md:block leading-[1.45rem] uppercase">
          SAFT
          <br />Apologetics
        </slide-reveal-text>
      </div>
      <div
        class="container my-14 md:my-40 flex md:grid grid-cols-2 row-start-2 col-span-full items-center place-content-center flex-wrap space-y-10 md:space-x-16"
      >
        <parallax-media class="w-full">
          <img src="@/assets/head.webp" alt="Head Image" />
        </parallax-media>
        <div>
          <slide-reveal-text as="h1" :delay="200" :triggered-by-scroll="true" :appear-once="true">
            Seeking Answers,
            Finding Truth.
          </slide-reveal-text>
          <p
            class="mt-4 max-w-[45ch] text-lg"
          >We are a web-based Christian initiative that aims to equip the believer defend their faith so that they may effectively evangelise.</p>
        </div>
      </div>
    </div>
  </div>
